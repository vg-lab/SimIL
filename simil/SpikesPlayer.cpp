                            ...
                            const std::string &activityPath)
{
  ...
  simil::SimulationData *simData = importer->LoadSimulationData(networkPath,
                                                                activityPath);
...
{
  StorageList storagelist = _simData->getStorage("Spikes");
  return dynamic_cast<StorageSparse *>(storagelist[0])->spikes(); ///TODO
}

StorageSparse *SpikesPlayer::spikeReport(void) const
{
  StorageList storagelist = _simData->getStorage("Spikes");
  if (storagelist.size() > 0)
    return dynamic_cast<StorageSparse *>(storagelist[0]); ///TODO
  return nullptr;
}
...
StorageSparse *SpikesPlayer::data(void) const
{
  StorageList storagelist = _simData->getStorage("Spikes");
  return dynamic_cast<StorageSparse *>(storagelist[0]);
}
} // namespace simil
