...
namespace boost
{
  namespace system
  {
    template<> struct is_error_code_enum< http_errors::http_error_codes >
    {
...
private:
  ...
  void on_request_sent( const boost::system::error_code& ec, std::size_t )
  {
    if ( ec != 0 )
    {
      on_finish( ec );
